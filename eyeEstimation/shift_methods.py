...
"""
Originally written by M. Petersen (Spring 2018); updated ~2019 by S. Betti.
Based on original implementation for ASTR 337 by M. Petersen, J. Lowenthal, K. Ward-Duong.
Usage:
            xshift, yshift = cross_image(reference_image, image_to_be_shifted, centerx, centery, boxsize=400)
            output_image = shift_image(image_to_be_shifted, xshift, yshift)
    inputs
    ----------------
    reference_image     : (matrix of floats) input image as reference
    image_to_be_shifted : (matrix of floats) input image to apply shift to
    centerx             : (float) x-center of subregion in reference image to shift to
    centery             : (float) y-center of subregion in reference image to shift to
    boxsize             : (integer) subregion of image to cross-correlate
    xshift              : (float) calculated offset in the x-axis
    yshift              : (float) calculated offset in the y-axis
...
def centroid(data_arr,xcen,ycen,nhalf=5,derivshift=1.):
    ...
    centroid
    -----------------
    based on dimension-independent line minimization algorithms implemented in IDL cntrd.pro
...
def cross_image(im1, im2, centerx, centery, boxsize=400, **kwargs):
    ...
    im1_gray = im1_gray[centery-boxsize:centery+boxsize,centerx-boxsize:centerx+boxsize]
    im2_gray = im2_gray[centery-boxsize:centery+boxsize,centerx-boxsize:centerx+boxsize]
