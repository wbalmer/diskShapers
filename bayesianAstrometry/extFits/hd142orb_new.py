...
if __name__=="__main__":
    ...
    savefile = 'rel_abs_posterior.hdf5'
    ...
    orb.savefig('HD142527_panelplot.pdf')
    ...
    corner_figure_median_95.savefig('hd142_postplot.pdf')
    print('Saved basic corner plot!')

    # seaborn posterior plot
    params = ['a$_{B}$ [au]','e$_{B}$','i$_{B}$ [$^\circ$]', '$\\omega_{B}$ [$^\circ$]', '$\\Omega_{B}$ [$^\circ$]','$\\tau_{B}$+0.5','$\\pi$ [mas]', '$\\mu_\\alpha$', '$\\mu_\\delta$', '$\\alpha_{A}^*$', '$\\delta_{A}$','M$_B$ [M$_{{\\odot}}$]','M$_A$ [M$_{{\\odot}}$]']
    import pandas as pd
    postframe = pd.DataFrame(myResults.post, columns=params)
    
    #convert radians to deg
    postframe['i$_{B}$ [$^\circ$]'] = np.rad2deg(postframe['i$_{B}$ [$^\circ$]']) % 360
    postframe['$\\omega_{B}$ [$^\circ$]'] = np.rad2deg(postframe['$\\omega_{B}$ [$^\circ$]']) % 360
    postframe['$\\Omega_{B}$ [$^\circ$]'] = np.rad2deg(postframe['$\\Omega_{B}$ [$^\circ$]']) % 360
    postframe['$\\tau_{B}$+0.5'] = (postframe['$\\tau_{B}$+0.5'] + 0.5) % 1
    ...
    g2 = sns.PairGrid(postframe,
                 vars = ['a$_{B}$ [au]','e$_{B}$','i$_{B}$ [$^\circ$]','M$_A$ [M$_{{\\odot}}$]','M$_B$ [M$_{{\\odot}}$]'],
                 diag_sharey=False, corner=True,
                 )
